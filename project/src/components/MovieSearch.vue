<template>
    <v-form>
        <v-container class="d-flex flex-row ga-2 pa-0">
            <v-text-field v-model="formData.query" label="Search Movie" placeholder="E.g Inception" variant="outlined"/>
            <v-select v-model="formData.type" :items="types" item-title="text" item-value="value" label="Type"
                placeholder="E.g Movie, series, episode" variant="outlined"/>
            <v-number-input v-model="formData.year" label="Year" placeholder="E.g 1997" variant="outlined":min="1950" :max="new Date().getFullYear()"/>
        </v-container>
